...
class DPOEvaluator:
    ...
    # logging buffers (one entry per DPO update)
        self.history = {
            "loss": [],          # preference loss per update
            "pairs": [],         # number of (w,l) pairs used in that update
            "margin": [],        # mean score(w) - score(l) on used pairs
            "entropy": [],       # H(pi) after the update
            "exp_prob": [],      # policy-expected BEM prob on a fresh batch
            "kl_ref": []         # KL(pi || pref) if not reference_free, else None
        }
    ...
    def __call__(self, prompts, examples, task, predictor, scorer, **_):
        ...
        for _ in range(self.eval_rounds):
            ...
            all_preds = [scorer._predict(batch, p) for p in prompts]  # B x N
            #for ex in batch:
            for j in range(B):
                #  get one prediction per prompt
                #preds = [predictor.inference(ex, p) for p in prompts]
                scores_j = [scorer.pair_prob(all_preds[i][j], batch[j]["answer"], batch[j]["question"]) for i in range(N)]

                # turn them into BEM scores
                #scores = [scorer.pair_prob(pred, ex["answer"], ex["question"]) for pred in preds]

                w = int(max(range(N), key=scores_j.__getitem__))   # best prompt
                #l_choices = [i for i in range(N) if i != w]
                #l = random.choice(l_choices)                    # random loser
                l = int(max((i for i in range(N) if i != w), key=scores_j.__getitem__))
                gap = scores_j[w] - scores_j[l]
                if gap < self.margin_tau:
                    continue
                winners.append(w); losers.append(l); margins.append(gap)
                # if scores[w] - scores[l] < self.margin_tau:
                #     continue                                    # skip near-ties
                # winners.append(w); losers.append(l)
                # margins.append(scores[w] - scores[l])
