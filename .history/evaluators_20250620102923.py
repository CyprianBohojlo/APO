...
class PPOEvaluator:
    ...
    def reward(self, prompt, examples, predictor, scorer):
        preds = predictor(prompt, [ex["guestion"] for ex in examples])
        gold_answers = [ex["answer"] for ex in examples]
        return scorer(preds, gold_answers) # Returns mean accuracy
    
    def __call__(self, prompts, task, predictor, scorer, random_number=None):
        ...
        train_pool = task.train_set           # iterable of dicts
        ...
        for _ in range(self.eval_rounds):
            ...
            # compute reward (BEM accuracy on the minibatch)
            rew = self.reward(prompts[idx], batch, predictor, scorer)
