        ...
        for prompt in tqdm(prompts, desc=f"expanding {len(prompts)} prompts"):
            ...
            if self.opt["n_gradients"] > 0:
                ...
                for idx, (feedback, err_str) in enumerate(gradients, start=1):
                    print(f"  Gradient {idx}:")
                    print("    feedback:", feedback.replace('\n','\\n'))
                    print("    err_str :", err_str.replace('\n','\\n'))
                print(">>> end gradients\n")
            ...
            if self.opt["mc_samples_per_step"] > 0:
                for sect in tqdm(new_task_sections + [task_section], desc='mc samples'):
                    ...
                    print(f"\n>>> MC PARAPHRASES for section:\n{sect}\n")
                    for i, s in enumerate(syns, start=1):
                        print(f"  Synonym {i}:", s.replace('\n','\\n'))
                    print(">>> end MC paraphrases\n")
            ...
            print(f"\n=== expand_candidates (prompt #{prompts.index(prompt)+1} of {len(prompts)}) ===")
            print(f"Seed Task section:\n{task_section}\n")
            print(f"Produced {len(tmp_prompts)} candidates. Showing their Task slices:\n")
            for i, cand in enumerate(tmp_prompts, start=1):
                sects = utils.parse_sectioned_prompt(cand)
                ts   = sects.get("task", "<missing>").strip().replace("\n", "\\n")
                print(f"  [{i:2d}] TASK: {ts}")
            print("=== end expand debug ===\n")
