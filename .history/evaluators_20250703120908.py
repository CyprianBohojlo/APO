...
class DPOEvaluator:
    def __init__(self,
        ...
        self.dpo = DPO(
            n_actions      = None,            # will be set on first call
            beta           = dpo_beta,
            lr             = dpo_lr,
            hidden         = dpo_hidden,
    ...
    def __call__(self, prompts, examples,
        ...
        N = len(prompts)
        if self.dpo.policy is None or N != self.dpo.policy.actor[-1].out_features:
            self.dpo = DPO(n_actions=N,
                        beta=self.dpo.beta,
                        lr=self.dpo.optimizer.param_groups[0]["lr"],
                        hidden=self.dpo.policy.actor[0].out_features,
