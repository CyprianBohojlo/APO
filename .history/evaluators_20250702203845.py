...
class DPOEvaluator:
    """
    Re-uses the DPO trainer to rank prompt candidates.
    Preferences are built via BEM (winner = highest score).
    """
    def __init__(self, cfg):
        self.samples_per_eval = cfg["samples_per_eval"]
        self.eval_rounds      = cfg["eval_rounds"]
        self.margin_tau       = cfg.get("dpo_margin", 0.0)

        self.dpo = DPO(
            n_actions       = cfg["pool_size"],
            beta            = cfg.get("dpo_beta", 0.1),
            lr              = cfg.get("dpo_lr", 3e-4),
            hidden          = cfg.get("dpo_hidden", 128),
            reference_free  = cfg.get("dpo_reference_free", False),
        )

    def __call__(self, prompts, examples,
                 task, predictor, scorer,
                 **ignored_kwargs):
        ...
        for _ in range(self.eval_rounds):
            ...
            for ex in batch:
                preds  = predictor.batch_inference([ex]*len(prompts), prompts)
                scores = scorer.batch_score(preds, ex)

                w = int(max(range(len(prompts)), key=scores.__getitem__))
                l_candidates = [i for i in range(len(prompts)) if i != w]
        ...
        # return preference weights for beam pruning
