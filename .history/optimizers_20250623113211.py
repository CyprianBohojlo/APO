        ...
        for prompt in tqdm(prompts, desc=f"expanding {len(prompts)} prompts"):
            ...
            if self.opt["n_gradients"] > 0:
                ...
                for idx, (feedback, err_str) in enumerate(gradients, start=1):
                    logger.debug(f"Gradient {idx} feedback={feedback.replace(chr(10),'\\n')}")
                    logger.debug(f"Gradient {idx} err_str ={err_str.replace(chr(10),'\\n')}")
            ...
            if self.opt["mc_samples_per_step"] > 0:
                for sect in tqdm(new_task_sections + [task_section], desc='mc samples'):
                    ...
                    logger.debug(f"<<< MC PARAPHRASES for section: {sect.replace(chr(10),'\\n')}")
                    for i, s in enumerate(syns,1):
                        logger.debug(f"   Synonym {i}: {s.replace(chr(10),'\\n')}")
            ...
            logger.debug(f"Seed Task section: {task_section.replace(chr(10),'\\n')}")
